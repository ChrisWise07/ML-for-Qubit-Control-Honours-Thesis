...
sequence_length_minimum_fidelities = {}
...
for sequence_length in [1, 2, 4, 8, 16, 32, 64, 128, 256, 512]:
    ...
    gates_optimisal_time_series = {}

    for gate in UNIVERSAL_GATE_SET_SINGLE_QUBIT.keys():
        random_time_series = (
            2 * torch.rand(150, sequence_length, number_of_control_channels)
            - 1
        ).to(DEVICE)

        ideal_process_matrix = IDEAL_PROCESS_MATRICES[gate]

        process_fidelity_scores = sim.time_series_to_process_fidelity_score(
            random_time_series, ideal_process_matrix.unsqueeze(0)
        )

        gate_fidelity_scores[gate] = process_fidelity_scores.mean().item()

    sequence_length_minimum_fidelities[sequence_length] = min(
        gate_fidelity_scores.values()
    )
...
for (
    sequence_length,
    minimum_fidelity,
) in sequence_length_minimum_fidelities.items():
    print(f"{sequence_length}: {round(minimum_fidelity,4)}")
