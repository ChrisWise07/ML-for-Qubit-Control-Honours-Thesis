...
class AxialPositionalEmbedding(nn.Module):
    def __init__(self, dim, axial_shape, axial_dims=None):
        ...
        for ind, (shape, axial_dim) in enumerate(zip(self.shape, axial_dims)):
            ...
            ax_emb = torch.zeros(ax_shape)
            nn.init.xavier_normal_(ax_emb)
            ax_emb = nn.Parameter(ax_emb)
...
class ParameterList(object):
    ...
    def to_list(self):
        return [
            getattr(self.kls, self._keyname(self.prefix, i))
...
class AxialPositionalEmbeddingImage(nn.Module):
    def __init__(self, dim, axial_shape, axial_dims=None):
        ...
        assert (
            len(axial_shape) == 2
        ), "Axial shape must have 2 dimensions for images"
