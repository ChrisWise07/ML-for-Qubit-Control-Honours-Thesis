...
class TruncatedMVN:
    ...
    def __init__(self, mu, cov, lb, ub, seed=None, print_debug=False):
        self.dim = len(mu)
    ...
    def colperm(self):
        ...
        for j in perm.copy():
            ...
            z[j] = (
                np.exp(-0.5 * tl**2 - w) - np.exp(-0.5 * tu**2 - w)
            ) / np.sqrt(2 * math.pi)
        return L, perm
