...
def hill_climb(
    ...
    current_sol = init_sol.clone()

    current_sol_score = objective_function(
        current_sol, ideal_expectations, sim
    )

    for _ in range(iterations):
        neighbor = mutate_control_pulse_sequence(current_sol, std)
        neighbor_score = objective_function(neighbor, ideal_expectations, sim)

        if neighbor_score < current_sol_score:
            current_sol = neighbor
            current_sol_score = neighbor_score

    return current_sol


def return_init_sol(
    init_sol_type: str, sequence_length: int, std: float, gate: str
):
    if "uniform_distro" == init_sol_type:
        return (
            2
            * torch.rand((1, sequence_length, number_of_control_channels)).to(
                DEVICE
            )
            - 1
        )
    ...
    if "normal_distro" == init_sol_type:
        return mutate_control_pulse_sequence(
            torch.zeros((1, sequence_length, number_of_control_channels)).to(
                DEVICE
            ),
            std=std,
        )
    ...
    for gate in UNIVERSAL_GATE_SET_SINGLE_QUBIT.keys():
        ...
        ideal_expectations = torch.cat(
            (
                ...
                ideal_expectations[:, 12:],
            ),
            dim=-1,
        )

        init_sol = return_init_sol(
        ...
        optimized_population_hc = hill_climb(
            init_sol=init_sol,
        ...
        final_fidelity_scores = sim.time_series_to_process_fidelity_score(
            time_series=optimized_population_hc,
            ideal_process_matrix=ideal_process_matrix,
        )

        fidelity_scores[gate] = final_fidelity_scores.item()
        optimal_pulse_sequences[gate] = optimized_population_hc.squeeze()
